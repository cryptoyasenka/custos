...
async function main(): Promise<void> {
  ...
  if (!pdaArg) {
    console.error("usage: npm run smoke:weaken -- <multisigPda>");
    process.exit(1);
  }
  ...
  console.log(`[smoke] current threshold: ${before.threshold}`);
  console.log(`[smoke] target threshold:  ${NEW_THRESHOLD}`);

  if (before.threshold === NEW_THRESHOLD) {
    console.log("[smoke] threshold already at target; nothing to do.");
    return;
  }

  const ix = multisig.instructions.multisigChangeThreshold({
    multisigPda,
  ...
  console.log(`[smoke] weaken tx: ${sig}`);

  const after = await multisig.accounts.Multisig.fromAccountAddress(connection, multisigPda);
  console.log(`[smoke] new threshold:     ${after.threshold}`);
  console.log("");
  console.log("If the daemon was watching, it should now have printed:");
  console.log(
    `  HIGH     [squads-multisig-weakening] Threshold weakened ${before.threshold} -> ${after.threshold} on multisig ${multisigPda.toBase58()}`,
  );
}
